...
└── src/cycle_tracker/
    ├── __init__.py
    ├── __main__.py           # `python -m cycle_tracker` / `uv run cycle-tracker`
...
uv run python -m cycle_tracker.seed   # optional: load sample data
uv run flet run main.py               # desktop window
uv run flet run --web main.py         # browser
uv run cycle-tracker                  # console-script entry point
```
...
- Daily symptom logging (separate from period events).
- Calendar / heat-map view.
- Cloud sync or export (CSV / iSync-style integration).
